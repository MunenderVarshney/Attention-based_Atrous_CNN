...
class EmbeddingLayers_pooling(nn.Module):
    def __init__(self):
        ...
        self.conv7 = nn.Conv2d(in_channels=128, out_channels=128,
                               kernel_size=(3, 3), stride=(1, 1),  dilation=1,
                               padding=(1, 1), bias=False)
        self.conv8 = nn.Conv2d(in_channels=256, out_channels=128,
                               kernel_size=(3, 3), stride=(1, 1),  dilation=1,
                               padding=(1, 1), bias=False)
       # 4 block , two pooling 
        ...
        self.conv11 = nn.Conv2d(in_channels=256, out_channels=256,
                               kernel_size=(1, 1), stride=(1, 1),  dilation=1,
                               padding=(1, 1), bias=False)
        self.conv12 = nn.Conv2d(in_channels=256, out_channels=256,
                               kernel_size=(1, 1), stride=(1, 1),  dilation=1,
                               padding=(1, 1), bias=False)

        self.conv13 = nn.Conv2d(in_channels=256, out_channels=512,
                               kernel_size=(1, 1), stride=(1, 1),  dilation=1,
                               padding=(1, 1), bias=False)
        self.conv14 = nn.Conv2d(in_channels=512, out_channels=512,
                               kernel_size=(1, 1), stride=(1, 1),  dilation=1,
                               padding=(1, 1), bias=False)                               
    ...
    def init_weights(self):
        ...
        init_layer(self.conv13)
        init_layer(self.conv14)
        init_bn(self.bn1)
        init_bn(self.bn2)
        init_bn(self.bn3)
        init_bn(self.bn4)
    ...
    def forward(self, input, return_layers=False):
        ...
        x = F.relu(self.bn1(self.short(x)))
        x = F.relu(self.bn2(self.conv5(x)))
        identity = x
        x = F.relu(self.bn2(self.conv6(x)) + identity)
        identity = x
        x = F.relu(self.bn2(self.conv7(x)))
        x = F.relu(self.bn2(self.conv8(x)) + identity)  
        x = F.relu(self.bn2(self.short1(x)))
        x = F.relu(self.bn3(self.conv9(x)))
        identity = x
        x = F.relu(self.bn3(self.conv10(x)) + identity) 
        identity = x
        x = F.relu(self.bn3(self.conv11(x)))
        x = F.relu(self.bn3(self.conv12(x))+ identity)
        identity = x
        x = F.relu(self.bn4(self.conv13(x)))
        x = F.relu(self.bn4(self.conv14(x))+ identity) 
